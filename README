...
properly do that is not fun. Also the Web MIDI API makes it a hard to react upon
receiving MIDI messages from external devices. For example, it only allows a single
callback function per channel and does not provide any help decoding the incoming binary
messages. The goal behind the WebMidi library was to makes these things easier.
...
install the *webmidi* package with:

```javascript
npm install webmidi
```

Otherwise, just grab the `webmidi-x.x.x.min.js` file from
[GitHub](https://github.com/cotejp/webmidi). You can find it in the `dist` folder. Save
this to your project and link it to your HTML page as usual.
...
and, optionnally, another callback function to execute upon failure. The function will
fail if the host browser does not suppport the Web MIDI API (currently, only Google Chrome
does) or if the browser can connect to the host's MIDI subsystem.

Let's check if WebMidi can be enabled. Link your page to `webmidi-x.x.x.min.js` and write

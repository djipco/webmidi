window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "api",
          "help",
          "index",
          "javascript",
          "librari",
          "midi",
          "readm",
          "tame",
          "web",
          "webmidi.j"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "list_class.html": [
          "class",
          "document",
          "list",
          "list:class"
        ],
        "WebMidi.html": [
          "alreadi",
          "api",
          "basic",
          "call",
          "cj",
          "class",
          "commonj",
          "directli",
          "easier",
          "es6",
          "esm",
          "global",
          "iif",
          "incom",
          "instanti",
          "librari",
          "make",
          "mean",
          "messag",
          "midi",
          "modul",
          "need",
          "new",
          "object",
          "outgo",
          "react",
          "send",
          "simpli",
          "simplifi",
          "us",
          "version",
          "web",
          "webmidi",
          "webmidi.j",
          "work"
        ],
        "WebMidi.html#enabled": [
          "access",
          "activ",
          "boolean",
          "enabl",
          "host'",
          "indic",
          "lt;readonly&gt",
          "member",
          "midi",
          "subsystem",
          "webmidi#en",
          "whether"
        ],
        "WebMidi.html#interface": [
          "api",
          "directli",
          "do",
          "instanc",
          "interfac",
          "know",
          "lt;readonli",
          "member",
          "midi",
          "midiaccess",
          "nullable&gt",
          "talk",
          "unless",
          "us",
          "web",
          "webmidi#interfac"
        ],
        "WebMidi.html#supported": [
          "actual",
          "api",
          "avail",
          "boolean",
          "built",
          "environ",
          "even",
          "exampl",
          "function",
          "indic",
          "instal",
          "lt;readonly&gt",
          "member",
          "midi",
          "navigator.requestmidiaccess",
          "note",
          "offer",
          "plugin",
          "properti",
          "provid",
          "report",
          "support",
          "though",
          "true",
          "web",
          "webmidi#support",
          "webmidiapishim.j",
          "whether"
        ],
        "WebMidi.html#enable": [
          "2020",
          "3",
          "access",
          "altogeth",
          "alway",
          "api",
          "approach",
          "april",
          "asynchron",
          "author",
          "avail",
          "base",
          "browser",
          "callback",
          "caus",
          "check",
          "chrome",
          "connect",
          "current",
          "display",
          "e.g",
          "enabl",
          "encourag",
          "environ",
          "exclus",
          "file",
          "function",
          "futur",
          "host",
          "host'",
          "http",
          "ignor",
          "implement",
          "import",
          "jazz",
          "librari",
          "localhost",
          "lt;async&gt",
          "matter",
          "messag",
          "method",
          "midi",
          "note",
          "omit",
          "oper",
          "option",
          "origin",
          "page",
          "paramet",
          "plugin",
          "promis",
          "prompt",
          "proof",
          "return",
          "secur",
          "set",
          "softwar",
          "start",
          "subsystem",
          "such",
          "support",
          "synth",
          "synthes",
          "sysex",
          "system",
          "tri",
          "true",
          "under",
          "us",
          "user",
          "v77",
          "version",
          "web",
          "webmidi#en"
        ]
      },
      "length": 8
    },
    "tokenStore": {
      "root": {
        "2": {
          "0": {
            "2": {
              "0": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              },
              "docs": {}
            },
            "docs": {}
          },
          "docs": {}
        },
        "3": {
          "docs": {
            "WebMidi.html#enable": {
              "ref": "WebMidi.html#enable",
              "tf": 0.46728971962616817
            }
          }
        },
        "docs": {},
        "a": {
          "docs": {},
          "p": {
            "docs": {},
            "i": {
              "docs": {
                "index.html": {
                  "ref": "index.html",
                  "tf": 10
                },
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                },
                "WebMidi.html#interface": {
                  "ref": "WebMidi.html#interface",
                  "tf": 4.166666666666666
                },
                "WebMidi.html#supported": {
                  "ref": "WebMidi.html#supported",
                  "tf": 1.7241379310344827
                },
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.9345794392523363
                }
              }
            },
            "p": {
              "docs": {},
              "r": {
                "docs": {},
                "o": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "h": {
                        "docs": {
                          "WebMidi.html#enable": {
                            "ref": "WebMidi.html#enable",
                            "tf": 0.9345794392523363
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "i": {
                "docs": {},
                "l": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "r": {
              "docs": {},
              "e": {
                "docs": {},
                "a": {
                  "docs": {},
                  "d": {
                    "docs": {},
                    "i": {
                      "docs": {
                        "WebMidi.html": {
                          "ref": "WebMidi.html",
                          "tf": 1.9607843137254901
                        }
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "o": {
                "docs": {},
                "g": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "h": {
                        "docs": {
                          "WebMidi.html#enable": {
                            "ref": "WebMidi.html#enable",
                            "tf": 0.46728971962616817
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "w": {
              "docs": {},
              "a": {
                "docs": {},
                "y": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.9345794392523363
                    }
                  }
                }
              }
            }
          },
          "c": {
            "docs": {},
            "c": {
              "docs": {},
              "e": {
                "docs": {},
                "s": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "WebMidi.html#enabled": {
                        "ref": "WebMidi.html#enabled",
                        "tf": 7.142857142857142
                      },
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "v": {
                  "docs": {
                    "WebMidi.html#enabled": {
                      "ref": "WebMidi.html#enabled",
                      "tf": 7.142857142857142
                    }
                  }
                }
              },
              "u": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            }
          },
          "v": {
            "docs": {},
            "a": {
              "docs": {},
              "i": {
                "docs": {},
                "l": {
                  "docs": {
                    "WebMidi.html#supported": {
                      "ref": "WebMidi.html#supported",
                      "tf": 1.7241379310344827
                    },
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.9345794392523363
                    }
                  }
                }
              }
            }
          },
          "s": {
            "docs": {},
            "y": {
              "docs": {},
              "n": {
                "docs": {},
                "c": {
                  "docs": {},
                  "h": {
                    "docs": {},
                    "r": {
                      "docs": {},
                      "o": {
                        "docs": {},
                        "n": {
                          "docs": {
                            "WebMidi.html#enable": {
                              "ref": "WebMidi.html#enable",
                              "tf": 0.46728971962616817
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "h": {
                "docs": {},
                "o": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "h": {
          "docs": {},
          "e": {
            "docs": {},
            "l": {
              "docs": {},
              "p": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 10
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.9345794392523363
                  }
                },
                "'": {
                  "docs": {
                    "WebMidi.html#enabled": {
                      "ref": "WebMidi.html#enabled",
                      "tf": 7.142857142857142
                    },
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "t": {
              "docs": {},
              "p": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              },
              "i": {
                "docs": {},
                "c": {
                  "docs": {
                    "WebMidi.html#enabled": {
                      "ref": "WebMidi.html#enabled",
                      "tf": 7.142857142857142
                    },
                    "WebMidi.html#supported": {
                      "ref": "WebMidi.html#supported",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "o": {
                "docs": {},
                "m": {
                  "docs": {
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 0.9803921568627451
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {},
                "a": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "i": {
                        "docs": {
                          "WebMidi.html": {
                            "ref": "WebMidi.html",
                            "tf": 2.941176470588235
                          }
                        }
                      }
                    },
                    "c": {
                      "docs": {
                        "WebMidi.html#interface": {
                          "ref": "WebMidi.html#interface",
                          "tf": 4.166666666666666
                        }
                      }
                    }
                  },
                  "l": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {},
                  "f": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "c": {
                        "docs": {
                          "WebMidi.html#interface": {
                            "ref": "WebMidi.html#interface",
                            "tf": 675
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "f": {
              "docs": {
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                }
              }
            }
          },
          "g": {
            "docs": {},
            "n": {
              "docs": {},
              "o": {
                "docs": {},
                "r": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "m": {
            "docs": {},
            "p": {
              "docs": {},
              "l": {
                "docs": {},
                "e": {
                  "docs": {},
                  "m": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "t": {
                          "docs": {
                            "WebMidi.html#enable": {
                              "ref": "WebMidi.html#enable",
                              "tf": 0.46728971962616817
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "o": {
                "docs": {},
                "r": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "j": {
          "docs": {},
          "a": {
            "docs": {},
            "v": {
              "docs": {},
              "a": {
                "docs": {},
                "s": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "r": {
                      "docs": {},
                      "i": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "t": {
                            "docs": {
                              "index.html": {
                                "ref": "index.html",
                                "tf": 10
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "z": {
              "docs": {},
              "z": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "b": {
              "docs": {},
              "r": {
                "docs": {},
                "a": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "i": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 10
                        },
                        "WebMidi.html": {
                          "ref": "WebMidi.html",
                          "tf": 0.9803921568627451
                        },
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_class.html": {
                    "ref": "list_class.html",
                    "tf": 110
                  }
                },
                ":": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "a": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "list_class.html": {
                                "ref": "list_class.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            ";": {
              "docs": {},
              "r": {
                "docs": {},
                "e": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "d": {
                      "docs": {},
                      "o": {
                        "docs": {},
                        "n": {
                          "docs": {},
                          "l": {
                            "docs": {},
                            "y": {
                              "docs": {},
                              "&": {
                                "docs": {},
                                "g": {
                                  "docs": {},
                                  "t": {
                                    "docs": {
                                      "WebMidi.html#enabled": {
                                        "ref": "WebMidi.html#enabled",
                                        "tf": 33.33333333333333
                                      },
                                      "WebMidi.html#supported": {
                                        "ref": "WebMidi.html#supported",
                                        "tf": 33.33333333333333
                                      }
                                    }
                                  }
                                }
                              }
                            },
                            "i": {
                              "docs": {
                                "WebMidi.html#interface": {
                                  "ref": "WebMidi.html#interface",
                                  "tf": 25
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "a": {
                "docs": {},
                "s": {
                  "docs": {},
                  "y": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "c": {
                        "docs": {},
                        "&": {
                          "docs": {},
                          "g": {
                            "docs": {},
                            "t": {
                              "docs": {
                                "WebMidi.html#enable": {
                                  "ref": "WebMidi.html#enable",
                                  "tf": 25
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "a": {
                "docs": {},
                "l": {
                  "docs": {},
                  "h": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "s": {
                        "docs": {},
                        "t": {
                          "docs": {
                            "WebMidi.html#enable": {
                              "ref": "WebMidi.html#enable",
                              "tf": 0.46728971962616817
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "i": {
            "docs": {},
            "d": {
              "docs": {},
              "i": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 10
                  },
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 2.941176470588235
                  },
                  "WebMidi.html#enabled": {
                    "ref": "WebMidi.html#enabled",
                    "tf": 7.142857142857142
                  },
                  "WebMidi.html#interface": {
                    "ref": "WebMidi.html#interface",
                    "tf": 4.166666666666666
                  },
                  "WebMidi.html#supported": {
                    "ref": "WebMidi.html#supported",
                    "tf": 3.4482758620689653
                  },
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 1.8691588785046727
                  }
                },
                "a": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "WebMidi.html#interface": {
                                "ref": "WebMidi.html#interface",
                                "tf": 29.166666666666664
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "k": {
              "docs": {},
              "e": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "t": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "n": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "s": {
                "docs": {},
                "a": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "WebMidi.html": {
                        "ref": "WebMidi.html",
                        "tf": 1.9607843137254901
                      },
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.9345794392523363
                      }
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "b": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "WebMidi.html#enabled": {
                        "ref": "WebMidi.html#enabled",
                        "tf": 110
                      },
                      "WebMidi.html#interface": {
                        "ref": "WebMidi.html#interface",
                        "tf": 110
                      },
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 110
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "h": {
                "docs": {},
                "o": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "d": {
              "docs": {},
              "u": {
                "docs": {},
                "l": {
                  "docs": {
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 0.9803921568627451
                    }
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              },
              "c": {
                "docs": {},
                "t": {
                  "docs": {
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 0.9803921568627451
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {},
              "o": {
                "docs": {},
                "r": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "a": {
            "docs": {},
            "m": {
              "docs": {},
              "e": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 10
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "k": {
                "docs": {
                  "WebMidi.html#interface": {
                    "ref": "WebMidi.html#interface",
                    "tf": 4.166666666666666
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "o": {
              "docs": {},
              "u": {
                "docs": {},
                "g": {
                  "docs": {},
                  "h": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "u": {
              "docs": {},
              "e": {
                "docs": {
                  "WebMidi.html#supported": {
                    "ref": "WebMidi.html#supported",
                    "tf": 3.4482758620689653
                  },
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 1.4018691588785046
                  }
                }
              }
            },
            "i": {
              "docs": {
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.46728971962616817
                }
              }
            }
          }
        },
        "w": {
          "docs": {},
          "e": {
            "docs": {},
            "b": {
              "docs": {
                "index.html": {
                  "ref": "index.html",
                  "tf": 10
                },
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                },
                "WebMidi.html#interface": {
                  "ref": "WebMidi.html#interface",
                  "tf": 4.166666666666666
                },
                "WebMidi.html#supported": {
                  "ref": "WebMidi.html#supported",
                  "tf": 1.7241379310344827
                },
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.9345794392523363
                }
              },
              "m": {
                "docs": {},
                "i": {
                  "docs": {},
                  "d": {
                    "docs": {},
                    "i": {
                      "docs": {
                        "WebMidi.html": {
                          "ref": "WebMidi.html",
                          "tf": 1903.921568627451
                        }
                      },
                      ".": {
                        "docs": {},
                        "j": {
                          "docs": {
                            "index.html": {
                              "ref": "index.html",
                              "tf": 600
                            },
                            "WebMidi.html": {
                              "ref": "WebMidi.html",
                              "tf": 0.9803921568627451
                            }
                          }
                        }
                      },
                      "#": {
                        "docs": {},
                        "e": {
                          "docs": {},
                          "n": {
                            "docs": {
                              "WebMidi.html#enabled": {
                                "ref": "WebMidi.html#enabled",
                                "tf": 1150
                              },
                              "WebMidi.html#enable": {
                                "ref": "WebMidi.html#enable",
                                "tf": 1150
                              }
                            }
                          }
                        },
                        "i": {
                          "docs": {},
                          "n": {
                            "docs": {},
                            "t": {
                              "docs": {},
                              "e": {
                                "docs": {},
                                "r": {
                                  "docs": {},
                                  "f": {
                                    "docs": {},
                                    "a": {
                                      "docs": {},
                                      "c": {
                                        "docs": {
                                          "WebMidi.html#interface": {
                                            "ref": "WebMidi.html#interface",
                                            "tf": 1150
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        },
                        "s": {
                          "docs": {},
                          "u": {
                            "docs": {},
                            "p": {
                              "docs": {},
                              "p": {
                                "docs": {},
                                "o": {
                                  "docs": {},
                                  "r": {
                                    "docs": {},
                                    "t": {
                                      "docs": {
                                        "WebMidi.html#supported": {
                                          "ref": "WebMidi.html#supported",
                                          "tf": 1150
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "a": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "i": {
                            "docs": {},
                            "s": {
                              "docs": {},
                              "h": {
                                "docs": {},
                                "i": {
                                  "docs": {},
                                  "m": {
                                    "docs": {},
                                    ".": {
                                      "docs": {},
                                      "j": {
                                        "docs": {
                                          "WebMidi.html#supported": {
                                            "ref": "WebMidi.html#supported",
                                            "tf": 1.7241379310344827
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "r": {
              "docs": {},
              "k": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "t": {
                "docs": {},
                "h": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "WebMidi.html#enabled": {
                          "ref": "WebMidi.html#enabled",
                          "tf": 7.142857142857142
                        },
                        "WebMidi.html#supported": {
                          "ref": "WebMidi.html#supported",
                          "tf": 1.7241379310344827
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "d": {
          "docs": {},
          "o": {
            "docs": {
              "WebMidi.html#interface": {
                "ref": "WebMidi.html#interface",
                "tf": 4.166666666666666
              }
            },
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "list_class.html": {
                            "ref": "list_class.html",
                            "tf": 35
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "r": {
              "docs": {},
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "i": {
                        "docs": {
                          "WebMidi.html": {
                            "ref": "WebMidi.html",
                            "tf": 0.9803921568627451
                          },
                          "WebMidi.html#interface": {
                            "ref": "WebMidi.html#interface",
                            "tf": 4.166666666666666
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "y": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      },
                      "WebMidi.html": {
                        "ref": "WebMidi.html",
                        "tf": 0.9803921568627451
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {},
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "list_class.html": {
                      "ref": "list_class.html",
                      "tf": 635
                    },
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 110.98039215686275
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                },
                "b": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "k": {
                        "docs": {
                          "WebMidi.html#enable": {
                            "ref": "WebMidi.html#enable",
                            "tf": 0.46728971962616817
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "u": {
              "docs": {},
              "s": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          },
          "j": {
            "docs": {
              "WebMidi.html": {
                "ref": "WebMidi.html",
                "tf": 0.9803921568627451
              }
            }
          },
          "o": {
            "docs": {},
            "m": {
              "docs": {},
              "m": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "j": {
                      "docs": {
                        "WebMidi.html": {
                          "ref": "WebMidi.html",
                          "tf": 0.9803921568627451
                        }
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "n": {
                "docs": {},
                "e": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "k": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "o": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "r": {
              "docs": {},
              "r": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "b": {
          "docs": {},
          "a": {
            "docs": {},
            "s": {
              "docs": {},
              "i": {
                "docs": {},
                "c": {
                  "docs": {
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 0.9803921568627451
                    }
                  }
                }
              },
              "e": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "l": {
                "docs": {},
                "e": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "WebMidi.html#enabled": {
                          "ref": "WebMidi.html#enabled",
                          "tf": 33.33333333333333
                        },
                        "WebMidi.html#supported": {
                          "ref": "WebMidi.html#supported",
                          "tf": 33.33333333333333
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "i": {
              "docs": {},
              "l": {
                "docs": {},
                "t": {
                  "docs": {
                    "WebMidi.html#supported": {
                      "ref": "WebMidi.html#supported",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {},
                "s": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "e": {
          "docs": {},
          "a": {
            "docs": {},
            "s": {
              "docs": {},
              "i": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "WebMidi.html": {
                        "ref": "WebMidi.html",
                        "tf": 0.9803921568627451
                      }
                    }
                  }
                }
              }
            }
          },
          "s": {
            "6": {
              "docs": {
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                }
              }
            },
            "docs": {},
            "m": {
              "docs": {
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                }
              }
            }
          },
          "n": {
            "docs": {},
            "a": {
              "docs": {},
              "b": {
                "docs": {},
                "l": {
                  "docs": {
                    "WebMidi.html#enabled": {
                      "ref": "WebMidi.html#enabled",
                      "tf": 683.3333333333334
                    },
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 676.4018691588785
                    }
                  }
                }
              }
            },
            "v": {
              "docs": {},
              "i": {
                "docs": {},
                "r": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "WebMidi.html#supported": {
                          "ref": "WebMidi.html#supported",
                          "tf": 3.4482758620689653
                        },
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.9345794392523363
                        }
                      }
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "o": {
                "docs": {},
                "u": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "g": {
                        "docs": {
                          "WebMidi.html#enable": {
                            "ref": "WebMidi.html#enable",
                            "tf": 0.46728971962616817
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "v": {
            "docs": {},
            "e": {
              "docs": {},
              "n": {
                "docs": {
                  "WebMidi.html#supported": {
                    "ref": "WebMidi.html#supported",
                    "tf": 1.7241379310344827
                  }
                }
              }
            }
          },
          "x": {
            "docs": {},
            "a": {
              "docs": {},
              "m": {
                "docs": {},
                "p": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "l": {
                "docs": {},
                "u": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.9345794392523363
                      }
                    }
                  }
                }
              }
            }
          },
          ".": {
            "docs": {},
            "g": {
              "docs": {
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.46728971962616817
                }
              }
            }
          }
        },
        "n": {
          "docs": {},
          "e": {
            "docs": {},
            "e": {
              "docs": {},
              "d": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                }
              }
            },
            "w": {
              "docs": {
                "WebMidi.html": {
                  "ref": "WebMidi.html",
                  "tf": 0.9803921568627451
                }
              }
            }
          },
          "u": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {},
                "a": {
                  "docs": {},
                  "b": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "&": {
                          "docs": {},
                          "g": {
                            "docs": {},
                            "t": {
                              "docs": {
                                "WebMidi.html#interface": {
                                  "ref": "WebMidi.html#interface",
                                  "tf": 25
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "v": {
              "docs": {},
              "i": {
                "docs": {},
                "g": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "o": {
                        "docs": {},
                        "r": {
                          "docs": {},
                          ".": {
                            "docs": {},
                            "r": {
                              "docs": {},
                              "e": {
                                "docs": {},
                                "q": {
                                  "docs": {},
                                  "u": {
                                    "docs": {},
                                    "e": {
                                      "docs": {},
                                      "s": {
                                        "docs": {},
                                        "t": {
                                          "docs": {},
                                          "m": {
                                            "docs": {},
                                            "i": {
                                              "docs": {},
                                              "d": {
                                                "docs": {},
                                                "i": {
                                                  "docs": {},
                                                  "a": {
                                                    "docs": {},
                                                    "c": {
                                                      "docs": {},
                                                      "c": {
                                                        "docs": {},
                                                        "e": {
                                                          "docs": {},
                                                          "s": {
                                                            "docs": {},
                                                            "s": {
                                                              "docs": {
                                                                "WebMidi.html#supported": {
                                                                  "ref": "WebMidi.html#supported",
                                                                  "tf": 1.7241379310344827
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "t": {
              "docs": {},
              "e": {
                "docs": {
                  "WebMidi.html#supported": {
                    "ref": "WebMidi.html#supported",
                    "tf": 1.7241379310344827
                  },
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.9345794392523363
                  }
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "b": {
            "docs": {},
            "j": {
              "docs": {},
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "WebMidi.html": {
                        "ref": "WebMidi.html",
                        "tf": 3.9215686274509802
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "g": {
                "docs": {},
                "o": {
                  "docs": {
                    "WebMidi.html": {
                      "ref": "WebMidi.html",
                      "tf": 0.9803921568627451
                    }
                  }
                }
              }
            }
          },
          "f": {
            "docs": {},
            "f": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {
                    "WebMidi.html#supported": {
                      "ref": "WebMidi.html#supported",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            }
          },
          "m": {
            "docs": {},
            "i": {
              "docs": {},
              "t": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          },
          "p": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.9345794392523363
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 26.869158878504674
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "i": {
              "docs": {},
              "g": {
                "docs": {},
                "i": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "e": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {
                  "WebMidi.html": {
                    "ref": "WebMidi.html",
                    "tf": 0.9803921568627451
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.9345794392523363
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.9345794392523363
                }
              }
            }
          },
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "WebMidi.html": {
                        "ref": "WebMidi.html",
                        "tf": 0.9803921568627451
                      }
                    },
                    "f": {
                      "docs": {},
                      "i": {
                        "docs": {
                          "WebMidi.html": {
                            "ref": "WebMidi.html",
                            "tf": 0.9803921568627451
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "b": {
              "docs": {},
              "s": {
                "docs": {},
                "y": {
                  "docs": {},
                  "s": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "m": {
                          "docs": {
                            "WebMidi.html#enabled": {
                              "ref": "WebMidi.html#enabled",
                              "tf": 7.142857142857142
                            },
                            "WebMidi.html#enable": {
                              "ref": "WebMidi.html#enable",
                              "tf": 0.46728971962616817
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {},
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "WebMidi.html#supported": {
                          "ref": "WebMidi.html#supported",
                          "tf": 688.5057471264369
                        },
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "h": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "f": {
              "docs": {},
              "t": {
                "docs": {},
                "w": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 1.4018691588785046
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "a": {
              "docs": {},
              "r": {
                "docs": {},
                "t": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "y": {
            "docs": {},
            "n": {
              "docs": {},
              "t": {
                "docs": {},
                "h": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  },
                  "e": {
                    "docs": {},
                    "s": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 1.4018691588785046
                    }
                  }
                }
              },
              "t": {
                "docs": {},
                "e": {
                  "docs": {},
                  "m": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.9345794392523363
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "s": {
            "docs": {
              "WebMidi.html": {
                "ref": "WebMidi.html",
                "tf": 3.9215686274509802
              },
              "WebMidi.html#interface": {
                "ref": "WebMidi.html#interface",
                "tf": 8.333333333333332
              },
              "WebMidi.html#enable": {
                "ref": "WebMidi.html#enable",
                "tf": 1.8691588785046727
              }
            },
            "e": {
              "docs": {},
              "r": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 1.4018691588785046
                  }
                }
              }
            }
          },
          "n": {
            "docs": {},
            "l": {
              "docs": {},
              "e": {
                "docs": {},
                "s": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "WebMidi.html#interface": {
                        "ref": "WebMidi.html#interface",
                        "tf": 4.166666666666666
                      }
                    }
                  }
                }
              }
            },
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          }
        },
        "v": {
          "7": {
            "7": {
              "docs": {
                "WebMidi.html#enable": {
                  "ref": "WebMidi.html#enable",
                  "tf": 0.46728971962616817
                }
              }
            },
            "docs": {}
          },
          "docs": {},
          "e": {
            "docs": {},
            "r": {
              "docs": {},
              "s": {
                "docs": {},
                "i": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "WebMidi.html": {
                          "ref": "WebMidi.html",
                          "tf": 1.9607843137254901
                        },
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.46728971962616817
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "k": {
          "docs": {},
          "n": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {
                  "WebMidi.html#interface": {
                    "ref": "WebMidi.html#interface",
                    "tf": 4.166666666666666
                  }
                }
              }
            }
          }
        },
        "f": {
          "docs": {},
          "u": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "t": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "WebMidi.html#supported": {
                            "ref": "WebMidi.html#supported",
                            "tf": 1.7241379310344827
                          },
                          "WebMidi.html#enable": {
                            "ref": "WebMidi.html#enable",
                            "tf": 110
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "l": {
              "docs": {},
              "e": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            }
          }
        },
        "p": {
          "docs": {},
          "l": {
            "docs": {},
            "u": {
              "docs": {},
              "g": {
                "docs": {},
                "i": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      },
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.46728971962616817
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "p": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "i": {
                        "docs": {
                          "WebMidi.html#supported": {
                            "ref": "WebMidi.html#supported",
                            "tf": 1.7241379310344827
                          }
                        }
                      }
                    }
                  }
                }
              },
              "v": {
                "docs": {},
                "i": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "WebMidi.html#supported": {
                        "ref": "WebMidi.html#supported",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              },
              "m": {
                "docs": {},
                "i": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 25.934579439252335
                      }
                    }
                  }
                },
                "p": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "WebMidi.html#enable": {
                        "ref": "WebMidi.html#enable",
                        "tf": 0.9345794392523363
                      }
                    }
                  }
                }
              },
              "o": {
                "docs": {},
                "f": {
                  "docs": {
                    "WebMidi.html#enable": {
                      "ref": "WebMidi.html#enable",
                      "tf": 0.46728971962616817
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "g": {
              "docs": {},
              "e": {
                "docs": {
                  "WebMidi.html#enable": {
                    "ref": "WebMidi.html#enable",
                    "tf": 0.46728971962616817
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "a": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "WebMidi.html#enable": {
                          "ref": "WebMidi.html#enable",
                          "tf": 0.9345794392523363
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      },
      "length": 181
    },
    "corpusTokens": [
      "2020",
      "3",
      "access",
      "activ",
      "actual",
      "alreadi",
      "altogeth",
      "alway",
      "api",
      "approach",
      "april",
      "asynchron",
      "author",
      "avail",
      "base",
      "basic",
      "boolean",
      "browser",
      "built",
      "call",
      "callback",
      "caus",
      "check",
      "chrome",
      "cj",
      "class",
      "commonj",
      "connect",
      "current",
      "directli",
      "display",
      "do",
      "document",
      "e.g",
      "easier",
      "enabl",
      "encourag",
      "environ",
      "es6",
      "esm",
      "even",
      "exampl",
      "exclus",
      "file",
      "function",
      "futur",
      "global",
      "help",
      "host",
      "host'",
      "http",
      "ignor",
      "iif",
      "implement",
      "import",
      "incom",
      "index",
      "indic",
      "instal",
      "instanc",
      "instanti",
      "interfac",
      "javascript",
      "jazz",
      "know",
      "librari",
      "list",
      "list:class",
      "localhost",
      "lt;async&gt",
      "lt;readonli",
      "lt;readonly&gt",
      "make",
      "matter",
      "mean",
      "member",
      "messag",
      "method",
      "midi",
      "midiaccess",
      "modul",
      "navigator.requestmidiaccess",
      "need",
      "new",
      "note",
      "nullable&gt",
      "object",
      "offer",
      "omit",
      "oper",
      "option",
      "origin",
      "outgo",
      "page",
      "paramet",
      "plugin",
      "promis",
      "prompt",
      "proof",
      "properti",
      "provid",
      "react",
      "readm",
      "report",
      "return",
      "secur",
      "send",
      "set",
      "simpli",
      "simplifi",
      "softwar",
      "start",
      "subsystem",
      "such",
      "support",
      "synth",
      "synthes",
      "sysex",
      "system",
      "talk",
      "tame",
      "though",
      "tri",
      "true",
      "under",
      "unless",
      "us",
      "user",
      "v77",
      "version",
      "web",
      "webmidi",
      "webmidi#en",
      "webmidi#interfac",
      "webmidi#support",
      "webmidi.j",
      "webmidiapishim.j",
      "whether",
      "work"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "WebMidi.js",
      "longname": "index",
      "name": "WebMidi.js",
      "tags": "index",
      "summary": "A JavaScript library to help you tame the Web MIDI API",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "list_class.html": {
      "id": "list_class.html",
      "kind": "list",
      "title": "Classes",
      "longname": "list:class",
      "name": "Classes",
      "tags": "list:class",
      "summary": "All documented classes.",
      "description": "",
      "body": ""
    },
    "WebMidi.html": {
      "id": "WebMidi.html",
      "kind": "class",
      "title": "WebMidi",
      "longname": "WebMidi",
      "name": "WebMidi",
      "tags": "WebMidi",
      "summary": "",
      "description": "The WebMidi object makes it easier to work with the Web MIDI API. Basically, it simplifies sending outgoing MIDI messages and reacting to incoming MIDI messages. When using the WebMidi.js library, the WebMidi class has already been instantiated for you. If you use the IIFE version, you should simply use the global object called WebMidi. If you use the CJS (CommonJS) or ESM (ES6 module) version, you get an already-instantiated object. This means there is no need to instantiate a new WebMidi object directly.",
      "body": ""
    },
    "WebMidi.html#enabled": {
      "id": "WebMidi.html#enabled",
      "kind": "member",
      "title": "&lt;readonly&gt; enabled :boolean",
      "longname": "WebMidi#enabled",
      "name": "enabled",
      "tags": "WebMidi#enabled enabled",
      "summary": "",
      "description": "Indicates whether access to the host's MIDI subsystem is active or not."
    },
    "WebMidi.html#interface": {
      "id": "WebMidi.html#interface",
      "kind": "member",
      "title": "&lt;readonly, nullable&gt; interface :MIDIAccess",
      "longname": "WebMidi#interface",
      "name": "interface",
      "tags": "WebMidi#interface interface",
      "summary": "",
      "description": "The MIDIAccess instance used to talk to the Web MIDI API. This should not be used directly unless you know what you are doing."
    },
    "WebMidi.html#supported": {
      "id": "WebMidi.html#supported",
      "kind": "member",
      "title": "&lt;readonly&gt; supported :boolean",
      "longname": "WebMidi#supported",
      "name": "supported",
      "tags": "WebMidi#supported supported",
      "summary": "",
      "description": "Indicates whether the environment provides support for the Web MIDI API or not. Note: in environments that do not offer built-in MIDI support, this will report true if the navigator.requestMIDIAccess function is available. For example, if you have installed WebMIDIAPIShim.js but no plugin, this property will be true even though actual support might not be there."
    },
    "WebMidi.html#enable": {
      "id": "WebMidi.html#enable",
      "kind": "function",
      "title": "&lt;async&gt; enable( [ options ] ) → {Promise}",
      "longname": "WebMidi#enable",
      "name": "enable",
      "tags": "WebMidi#enable enable",
      "summary": "",
      "description": "Checks if the Web MIDI API is available in the current environment and then tries to connect to the host's MIDI subsystem. This is an asynchronous operation and it causes a security prompt to be displayed to the user. Since version 3, this method returns a Promise. We encourage users to use the promise-based approach. When using such an approach, you can omit the callback option altogether. To enable the use of MIDI system exclusive messages, the sysex parameter should be set to true. However, under some environments (e.g. Jazz-Plugin), the sysex parameter is ignored and system exclusive messages are always enabled. To enable access to software synthesizers available on the host, you would set the software option to true. However, note that this option is only there to future-proof the library as support for software synths has not yet been implemented in any browser (April 2020). Important note: starting with Chrome v77, a page using Web MIDI API must be hosted on a secure origin (https://, localhost or file:///) and the user will always be prompted to authorize the operation (no matter if the sysex option is true or not)."
    }
  }
};
import {WebMidi} from "../../dist/webmidi.esm.min.js";
const expect = chai.expect;

describe("WebMidi", function() {

  it("should not allow itself to be instantiated in the regular manner", function() {
    expect(function() {
      new WebMidi();
    }).to.throw(TypeError);
  });
  ...
  describe("enable()", function() {
    ...
    // it(
    //   "should pass an error to the callback if the Web MIDI API is not supported (old syntax)",
    ...
    // it(
    //   "should pass an error to the callback if the Web MIDI API is not supported (new syntax)",
    //   sinon.test(function (done) {
    //
    //     if (navigator && navigator.requestMIDIAccess) {
    //       var rma = this.stub(navigator, "requestMIDIAccess");

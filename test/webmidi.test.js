...
describe("WebMidi", function() {
  ...
  it("should adapt to Electron, NW.js or pure Node.js environments", (done) =>
    done());
  ...
  describe("disable()", function() {
    ...
    it("should remove all user handlers", function() {
      for (var i = 0; i < WebMidi._midiInterfaceEvents.length; i++) {
        expect(
          WebMidi._userHandlers[WebMidi._midiInterfaceEvents[i]].length
        ).to.equal(0);
      }
    });
  ...
  describe("enable()", function() {
    ...
    it("should set 'enabled' property to false if it fails", function(done) {
      if (navigator && navigator.requestMIDIAccess) {
        ...
        navigator.requestMIDIAccess.returns(
    ...
    it("should create as many inputs and outputs as are available on the host", (done) =>
      done());
  });
  ...
  describe("getInputByName()", function() {
    ...
    it("should return the right input", function() {
      if (WebMidi.inputs.length > 0) {
        ...
        expect(WebMidi.getInputByName(name).name).to.equal(
          WebMidi.inputs[0].name
        );
      } else {
        this.skip();
      }
    ...
    it("should return instance of Output", function() {
      if (WebMidi.inputs.length > 0) {
        expect(
          ...
          WebMidi.inputs[WebMidi.inputs.length - 1].constructor
        );
      } else {
        this.skip();
      }
  ...
  describe("getOctave()", function() {
    ...
    it("should return undefined if number is invalid", function() {
      ["abc", null, undefined, -1, 128, function() {}, {}, "555"].forEach(
        function(param) {
  ...
  describe("getOutputByName()", function() {
    ...
    it("should return the right output", function() {
      if (WebMidi.outputs.length > 0) {
        ...
        expect(WebMidi.getOutputByName(name).name).to.equal(
          WebMidi.outputs[0].name
        );
      } else {
        this.skip();
      }
    ...
    it("should return instance of Output", function() {
      if (WebMidi.outputs.length > 0) {
        expect(
          WebMidi.getOutputByName(
            WebMidi.outputs[WebMidi.outputs.length - 1].name
          )
        ).to.be.instanceOf(
          WebMidi.outputs[WebMidi.outputs.length - 1].constructor
        );
      } else {
        this.skip();
      }
      ...
      [-20, -15, -9, 0, 7, 13, 20].forEach(function(offset) {
        ...
        for (var octave = -1 + offset; octave <= 9 + offset; octave++) {
          for (var k = 0; k < WebMidi._notes.length; k++) {
            if (value > 127) break;
            expect(
              WebMidi.guessNoteNumber(WebMidi._notes[k] + octave)
            ).to.equal(value);
            value++;
          }
        }

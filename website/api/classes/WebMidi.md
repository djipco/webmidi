...
example, if the `octaveOffset` is set to `-1` and a [`"noteon"`](Input#event:noteon) message
with MIDI number 60 comes in, the note will be reported as C3 (instead of C4).
